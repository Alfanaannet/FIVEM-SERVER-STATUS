...
const startBot = function (values) {
  console.log(
    `${chalk.bgBlue("[INFO]")} ${chalk.blue(
      "Starting bot... this may take a few seconds to start..."
    )}`
  );
  ...
  if (process.argv.includes("-c") || process.argv.includes("--config")) {
    setup.loadValues().then((values) => {
        ...
        console.log(
          "Unable to load saved values, reconfiguring all saved values again"
        );
    ...
    console.log(
      `${chalk.bold.bgYellow(`[LOAD]`)} ${chalk.bold.yellow(
        "Attempting to load enviroment values..."
      )}`
    );
    setup.loadValues().then((values) => {
        startBot(values);
